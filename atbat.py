...
page = st.sidebar.selectbox('Select a page:', ['At-bat Events', 'Compare Players', 'Calculations Explained'])

# At-bat Events
if page == 'At-bat Events':
    st.title('At-bat Events')

    # Input Data
    df = pd.read_csv('short_max_speed.csv', index_col=0)
    ...
    event_ids = df_person['eventId'].unique()
    selected_event_id = st.selectbox('Select an Event ID:', event_ids)


    df= df_person[df_person['eventId'] == selected_event_id]
    ...
    with col[0]:
        with st.expander("Player Info"):
            st.text( f"Team Id: {df['teamId.mlbId'].iloc[0]}")
            st.text( f"Handedness: {handedness}")
    # Display game score
    with col[1]:
        with st.expander("Game Score"):  
            st.table(new_df.style.format(precision=2).set_properties(**{
...
# Compare Players Page
    ...
    col = st.columns((2, 2), gap='medium')
    # Collect Player 1 and Player 2 events
    with col[0]:
        person_ids = df['personId.mlbId'].unique()
        selected_person_id = st.selectbox('Select Player 1:', person_ids)
        df_A = df[df['personId.mlbId'] == selected_person_id]
        event_ids_A = df_A['eventId'].unique()
        selected_event_id_A = st.selectbox('Select an Event ID for Player 1:', event_ids_A)

    with col[1]:
        remaining_person_ids = [pid for pid in person_ids if pid != selected_person_id]
        selected_person_id_B = st.selectbox('Select Player 2:', remaining_person_ids)
        df_B = df[df['personId.mlbId'] == selected_person_id_B]
        event_ids_B = df_B['eventId'].unique()
    ...
    if __name__ == "__main__":
        df_person1 = df_A[df_A['eventId'] == selected_event_id_A]
        df_person2 = df_B[df_B['eventId'] == selected_event_id_B]
        main(df_person1, df_person2)

# Def'ns
elif page == 'Calculations Explained':
    ...
    st.write('Swing: If the attack angle is between 0-4 degrees, the swing is level. If the attack angle is greater than 4, than the swing is upper cut, otherwise under cut.')
